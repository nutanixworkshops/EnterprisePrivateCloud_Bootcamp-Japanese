...
#. 確認ダイアログが行事されたら **OK** をクリックし、ルールの追加を完了します。
...
定義したポリシーを適用するためには、ポリシーを **適用(Apply)** する必要があります。
...
#. *Initials*\ **-WinToolsVM** コンソールに戻ります。
  ...
  クライアントからnodeサーバーへのpingトラフィックは継続しているが、データベースサーバーへのpingトラフィックがブロックされた事を確認します。
